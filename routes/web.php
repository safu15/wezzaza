...
Route::get('verify/account/{confirmation_code}', 'HomeController@getVerifyAccount')->where('confirmation_code','[A-Za-z0-9]+');
 ...
 Route::group(['middleware' => 'auth'], function() {
	 ...
	 // Settings Page
         Route::get('settings/pages','UserController@settingsPage');
	...
	Route::get('my/payments/invoice/{id}','UserController@invoice');
        ...
        //bank details
        Route::post('getSwift','UserController@getSwift');
...
Route::group(['middleware' => 'private.content'], function() {
	...
	Route::get('liveStreaming','UserController@liveStreaming')->name('liveStraming');
	Route::post('startStreaming','UserController@startStreaming')->name('startStreaming');
	Route::post('leaveStreaming','UserController@leaveStreaming')->name('leaveStreaming');
	Route::get('showLiveStreaming/{username?}','UserController@showLiveStreaming')->name('showLiveStreaming');
        Route::post('leavePage','UserController@leavePage')->name('leavePage');
        Route::post('requestCoHost','UserController@requestCoHost')->name('requestCoHost');
        Route::post('checkCoRequest','UserController@checkCoRequest')->name('checkCoRequest');
        Route::post('acceptCoHost','UserController@acceptCoHost')->name('acceptCoHost');
        Route::post('searchCoHost','UserController@searchCoHost')->name('searchCoHost');
        Route::post('acceptreq','UserController@acceptreq')->name('acceptreq');
        Route::post('leaveCoHost','UserController@leaveCoHost')->name('leaveCoHost');
        Route::post('hostleaveCohost','UserController@hostleaveCohost')->name('hostleaveCohost');
         Route::post('checkNotification','UserController@checkNotification')->name('checkNotification');
         Route::post('LivestreamUserCount','UserController@LivestreamUserCount')->name('LivestreamUserCount');
         Route::post('checkCountUser','UserController@checkCountUser')->name('checkCountUser');
         Route::post('sendLiveMessage','UserController@sendLiveMessage')->name('sendLiveMessage');
         Route::post('CheckCohostRemove','UserController@CheckCohostRemove')->name('CheckCohostRemove');
      
         Route::post('checkCohost','UserController@checkCohost')->name('checkCohost');

        Route::get('liveStream','UserController@liveStream')->name('liveStraming');
	
        // Upload Cover event
	Route::post('upload/eventimg','UserController@uploadEventImg');
	Route::post('upload/event','UserController@uploadEvent')->name('uploadEvent');
        
        //event
        Route::get('events','UserController@events')->name('events');
        Route::post('countAdd','UserController@countAdd')->name('countAdd');
 ...
 Route::group(['middleware' => 'role'], function() {
	...
	Route::post('panel/admin/settings/email','AdminController@emailSettings');
        ...
        // AGORA KEY SETTINGS
	Route::view('panel/admin/agora/key','admin.agora-key');
	Route::post('panel/admin/agora/key','AdminController@agora_key_modify');
